...
export async function deriveKillList(): Promise<KillTask[]> {
  ...
  for (const release of releases) {
    ...
    const EP_TRACKS = ['SEE ME', 'Sweet Frustration', 'East Side Love', 'Green Light', 'WANT U 2'];
  ...
  // Hard deadlines — EP upload by May 11, EP release May 15
  const epUploadDeadline = new Date("2026-05-11T00:00:00");
  ...
  // SF Mixdown — master must lock by Apr 30
  if (sfHours < 10 && now < epUploadDeadline) {
    ...
    tasks.push({
      ...
      subtitle: `Sweet Frustration: ${sfHours} / 10 hrs logged. ${daysToUpload} days to EP upload (May 11).`,
      howTo: [
  ...
  // LID Mixdown — REMOVED. Like I Did is vault-only, NOT on the EP.
  // (LID task removed Apr 27 — vault track, may surface post-EP)

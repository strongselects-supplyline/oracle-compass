...
// ── Simple sobriety link — full dashboard lives on /grind (Body tab) ──
function SovereigntyClockLink() {
  const [days, setDays] = React.useState<number | null>(null);

  React.useEffect(() => {
    import("@/lib/sovereignty").then(({ loadSovereigntyState, getSobrietyDays }) => {
      loadSovereigntyState().then(s => setDays(getSobrietyDays(s.sobrietyStart)));
    });
  }, []);
  ...
  return (
    <a
      href="/grind"
      className="flex items-center justify-between card mb-6 no-underline active:scale-[0.98] transition-all"
      style={{ borderColor: "var(--border)" }}
    >
      <div>
        <p className="text-[10px] font-black tracking-[0.18em] uppercase" style={{ color: "var(--text-muted)" }}>Sobriety Clock</p>
        <p className="text-2xl font-black mt-0.5" style={{ color: "var(--accent)" }}>
          {days !== null ? `Day ${days}` : "—"}
        </p>
      </div>
      <div className="flex items-center gap-1.5" style={{ color: "var(--text-muted)" }}>
        <span className="text-[10px] font-black tracking-widest uppercase">Body →</span>
      </div>
    </a>
  );
}
